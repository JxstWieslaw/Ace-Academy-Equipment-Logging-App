...
public class MembershipController {
    ...
    public TableView<MembershipModel> mem_tbl;
    ...
    public void initialize() throws ClassNotFoundException {
        ...
        try {
            ...
            selectall = connection.prepareStatement("SELECT * from membershipdetail");
            slectmemID = connection.prepareStatement("select * from membershipdetail where id=?");
            newIdQuery = connection.prepareStatement("SELECT id FROM membershipdetail");
            addToTable = connection.prepareStatement("INSERT INTO membershipdetail values(?,?,?,?)");
            updateQuarary = connection.prepareStatement("UPDATE membershipdetail SET name=? , address=? , contact=? where id=?");
            ObservableList<MembershipModel> members = mem_tbl.getItems();
            ResultSet rst = selectall.executeQuery();
            while (rst.next()) {
                System.out.println("load");
                members.add(new MembershipModel(
                        rst.getString(1),
                        rst.getString(2),
                        rst.getString(3),
                        rst.getString(4)
        ...
        mem_tbl.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<MembershipModel>() {
            @Override
            public void changed(ObservableValue<? extends MembershipModel> observable, MembershipModel oldValue, MembershipModel newValue) {
                MembershipModel selectedItem = mem_tbl.getSelectionModel().getSelectedItem();
                try {
                    connection = null;
                    try {
    ...
    public void btn_add(ActionEvent actionEvent) throws SQLException {
        ObservableList<MembershipModel> members = FXCollections.observableList(DB.members);
        //is empty
